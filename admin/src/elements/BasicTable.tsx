...
function DataTable({ data }) {
    ...
    return (
        <table style={{ borderCollapse: 'collapse' }}>
            <thead>
                <tr>
                    {data[0] && columns.map((heading: string) => <th style={{ padding: '0 3px' }} key={heading}>{heading.toUpperCase()}</th>)}
                </tr>
            </thead>
            <tbody>
                {
                    data.map(row =>
                        <tr key={row.testcase_id} style={{ borderBottom: '2px solid black' }}>{
                            columns.map(column => <td key={column} style={{ whiteSpace: 'pre-line' }}>{row[column] ?? '-'}</td>)
                        }
                        </tr>)
                }
            </tbody>
