import { Grid, TextField, InputAdornment, Box, Typography, TextareaAutosize, Button, Dialog, DialogContent, DialogTitle, Switch, Hidden } from "@mui/material";
...
export default function Ques(props) {
    ...
    const [testCase, setTestCase] = React.useState<any[]>([
        { input: '2,4', output: '4', points: 2, testcase_id: '43', hidden: false },
        { input: '2,4', output: '4', points: 2, testcase_id: '44', hidden: false }]);
    let codingQuestion = "";
    ...
    function Switch1(checkd, index) {

        const [checked, setChecked] = React.useState(checkd ?? false);

        const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
            setChecked(event.target.checked);
            const tc = testCase;
            tc[index].hidden = event.target.checked;
            setTestCase(tc)
                ...
                <Grid item md={5} sm={4} xs={12}>
                    <BasicTable
                        ...
                        columns={[
                            ...
                            {
                                field: "Hidden", flex: 1, headerAlign: "center", align: "center", sortable: false, disableColumnMenu: true,
                                renderCell: (params: GridRenderCellParams<any>) => (
                                    Switch1(params.row.Hidden, params.row.id)
                                )
                            }
                        ]}
                        deletedId={(id) => {
